"""V1.3 challenger: identical to worker/sports_nova_v3/simulator.py except
each team's sack_rate is a causal, training-derived per-team estimate
(team_rates.causal_sack_rate) instead of the frozen feature construction's
hardcoded .065 constant for every team. scramble_rate is left UNCHANGED
(no real per-team signal exists for it in this data source). Every other
mechanism is imported UNCHANGED from worker.sports_nova_v3.
"""
...
from .team_rates import causal_sack_rate
...
def _model_ref(pregame: PregameState) -> FrozenModelRef:
    raw = hashlib.sha256(b"SPORTS_NOVA_V1_3_ARCH_1:drive_block:causal_sack_rate:research").hexdigest()
...
def _run_one(pregame: PregameState, sim_id: int, seed: int, params, sack_rates: dict[str, float]):
    ...
    for _ in range(60):
        ...
        # SPORTS_NOVA_V1_3_CAUSAL_SACK_RATE: the only behavioral change vs
        # V3 -- sack_rate comes from this team's own causal history instead
        # of the frozen feature construction's hardcoded .065 for everyone.
        # scramble_rate is untouched (no real per-team signal available).
        selection = select_plays(plays, script_pass_rate(pregame, state, env), rng,
            sack_rate=sack_rates[offense],
...
def simulate_game(pregame_state: PregameState, n_sims: int, seed: int, model_version: str) -> SimulationBatch:
    ...
    # Computed ONCE per game (not per simulation draw): parse season/week
    # from game_id using the same convention scripts/sports_nova_v3_player_
    # joint_walkforward_v1.py's game_parts() uses, then look up each team's
    # causal sack rate.
    parts = str(pregame_state.game_id).split("_")
    season, week = int(parts[0]), int(parts[1])
    sack_rates = {tid: causal_sack_rate(tid, season, week) for tid in team_ids}
    ...
    for i in range(n_sims):
        stats, team, win = _run_one(pregame_state, i, seed, params, sack_rates)
        winners[i] = win
