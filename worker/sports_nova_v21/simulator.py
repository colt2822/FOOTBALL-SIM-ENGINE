"""V21 challenger: identical to worker/sports_nova_v19/simulator.py except
`_run_one`'s scoring-credit step now draws an extra-point (PAT) attempt per
touchdown instead of crediting a bare 6 points. See package docstring
(worker/sports_nova_v21/__init__.py) for the full SPORTS_V21_EXTRA_POINT_FIX
rationale and evidence.

Everything else -- QB identity/roster resolution (`_qb_shares`,
`set_identity_resolutions`), pace/block-volume, allocation, efficiency, TD
rates, catch rate -- is imported UNCHANGED from worker.sports_nova_v19 (which
itself imports unchanged from worker.sports_nova_v3 except `_qb_shares`),
not copied, so this is a single, attributable, diffable change.
"""
from __future__ import annotations

import numpy as np

from .config import MODEL_VERSION, RNG_ALGORITHM, PAT_MAKE_RATE
    ...
    for _ in range(60):
        ...
        for pid, targets in alloc.target_counts.items():
            ...
            if td_count:
                ...
                # SN3_V21_EXTRA_POINT_FIX: a touchdown alone is 6 points; the
                # extra-point attempt that always follows in real football
                # was previously never modeled (see package docstring).
        ...
        for pid, carries in alloc.carry_counts.items():
            ...
            if rush_td:
                ...
                # SN3_V21_EXTRA_POINT_FIX: same PAT-after-TD correction for
                # rushing touchdowns.
