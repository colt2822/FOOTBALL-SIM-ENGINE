"""Deterministic pregame game-eligibility for V24.  Pure functions: no IO, no market data, no outcomes."""
...
from .config import (
    DOUBTFUL_AVAILABILITY_WEIGHT, ELIGIBLE_ROSTER_STATUS, QUESTIONABLE_AVAILABILITY_WEIGHT,
    REASON_PRECEDENCE, ROSTER_STATUS_REASON)


@dataclass(frozen=True)
class RosterSnapshot:
    """Everything eligibility may read, all available pre-kickoff.
...
class PlayerEligibility:
    ...
    team_id: str
    eligible: bool
    reason: str | None              # primary reason code, None when eligible
    all_reasons: tuple[str, ...]
    availability_weight: float      # 1.0 unless an eligible Doubtful player
    detail: str
...
def classify_player(player, snapshot: RosterSnapshot) -> PlayerEligibility:
    ...
    weight = 1.0
    if not reasons:
        weight = {"DOUBTFUL": DOUBTFUL_AVAILABILITY_WEIGHT,
                  "QUESTIONABLE": QUESTIONABLE_AVAILABILITY_WEIGHT}.get(game_status, 1.0)
    ...
    return PlayerEligibility(player.player_id, player.team_id, not reasons, primary,
                             tuple(r for r in REASON_PRECEDENCE if r in reasons), weight, detail)
