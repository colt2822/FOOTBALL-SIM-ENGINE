"""V1.2 challenger: identical to worker/sports_nova_v3/simulator.py except
each receiver's `yards_per_reception` feature is empirical-Bayes shrunk
toward a training-derived league mean before being used as the per-catch
yardage mean. Every other mechanism (pace/block-volume, allocation, QB
shares, efficiency, scoring, TD rates, catch rate) is imported UNCHANGED
from worker.sports_nova_v3 -- not copied -- so this is a single,
attributable, diffable change, independent of worker/sports_nova_v1_1's
(rejected) pace experiment.
"""
...
# SPORTS_NOVA_V1_2_YPR_SHRINKAGE: worker/sports_nova_v3's `yards_per_reception`
# feature (scripts/sports_nova_v3_player_joint_walkforward_v1.py, `feat(...,
# (r.rec_y / r.rec) if r.rec else 10., ...)`) is a raw, UNSHRUNK per-player
# historical mean over an up-to-5-season lookback window -- a classic
# small-sample-forecast bias (see calibration.py, which only calibrates
# binary-outcome probabilities, never touches this continuous feature).
# Validated on TRAINING data only (NFL_V3_DRIVE_BLOCK_CANONICAL_V1.parquet
# player-panel equivalent, WR/TE/RB rows, with the 60-game
# SPORTS_NOVA_V1_1 evaluation cohort's games excluded): predicting a
# player's NEXT season's reception-weighted YPR from a games-weighted
# empirical-Bayes shrinkage of their prior-window raw YPR toward the
# training league mean reduces weighted out-of-sample MSE from 8.060 (no
# shrinkage, k=0) to 5.410 at k=12 games -- a 33% error reduction, stable
# across k in roughly [8, 20] (not a knife-edge fit to a specific k).
# LEAGUE_MEAN_YPR and SHRINKAGE_K below are exactly those training-derived
# constants; the eval cohort was never used to fit them.
LEAGUE_MEAN_YPR = 11.169479601207145
SHRINKAGE_K_GAMES = 12.0


def _shrunk_ypr(receiver, raw_ypr: float) -> float:
    n = max(float(receiver.uncertainty.effective_sample_size), 0.0)
    return (n * raw_ypr + SHRINKAGE_K_GAMES * LEAGUE_MEAN_YPR) / (n + SHRINKAGE_K_GAMES)
...
def _model_ref(pregame: PregameState) -> FrozenModelRef:
    # See worker/sports_nova_v1_1/simulator.py's identical comment: FrozenModelRef.version
    # is a schema Literal pinned to the shared ARCH_1 drive-block contract, left at its
    # schema default rather than overridden.
    raw = hashlib.sha256(b"SPORTS_NOVA_V1_2_ARCH_1:drive_block:ypr_shrinkage:research").hexdigest()
...
def _run_one(pregame: PregameState, sim_id: int, seed: int, params):
    ...
    for _ in range(60):
        ...
        for pid, targets in alloc.target_counts.items():
            ...
            # SPORTS_NOVA_V1_2_YPR_SHRINKAGE: the only behavioral change vs
            # V3 -- shrink the raw feature toward the training league mean
            # before using it as this receiver's per-catch yardage mean.
            raw_ypr_mean = _feature(receiver, "yards_per_reception", params.receiving_yards_mean)
            ypr_mean = _shrunk_ypr(receiver, raw_ypr_mean)
