"""SPORTS_V19_QB_IDENTITY_ONLY_CHALLENGER -- capture phase (predictions only).

Mirrors scripts/sports_nova_v3_v18_week1_replay_capture.py exactly (same
prior-panel construction, same frozen `make_state`, same seeds, same
N_SIMS, same 16 games) except it calls worker.sports_nova_v19.simulator
instead of worker.sports_nova_v3.simulator, with the season-boundary QB
identity resolutions (scripts/sports_nova_v19_qb_identity_resolver.py)
installed beforehand. Never imports the outcome-join script; never reads
home_score/away_score or any post-kickoff panel row.

Two-phase run:
  1. FIDELITY CHECK: re-simulate 2 games (the MNF canary + one more) through
     worker.sports_nova_v19.simulator with an EMPTY identity-resolution
     table installed (override never fires) and diff STATE_HASH/
     RAW_DRAWS_SHA256/MODEL_SCORE_DIST against the existing, already-
     hash-verified frozen V18 replay artifacts on disk. This proves the
     forked harness reproduces V18 byte-for-byte when the one permitted
     change is inert, i.e. that nothing else drifted in the fork -- before
     any real comparison is trusted.
  2. REAL RUN: all 16 games, with the CONFIRMED season-boundary
     resolutions installed. Every prediction artifact records whether the
     override actually changed `_primary_qb`'s pick for that team (by
     comparing against the frozen V18 artifact's own PREDICTED_QB_ID,
     already on disk) -- so "did the fix reach this slot" is auditable
     per-team, not just asserted.
"""
...
V18_DRAWS_DIR = V18_REPLAY_DIR / "draws"

REPLAY_DIR = DATA / "replay_v19" / "week1_2026"
...
def verify_freeze_and_load():
    ...
    spec = importlib.util.spec_from_file_location(
        "sports_nova_v3_player_joint_walkforward_v1_V19_REPLAY", WALKFORWARD_SCRIPT)
...
def run_one_game(walkforward_mod, v19_sim, panel, r, resolutions):
    ...
    def override_info(team, qb):
        rec = resolutions.get((game_id, team))
        v18_qb_id = None
        if v18_pred is not None:
            side = "HOME_QB" if team == home else "AWAY_QB"
            v18_qb_id = (v18_pred.get(side) or {}).get("PLAYER_ID")
        return {
            "RESOLUTION": rec,
            "V19_PICKED_QB_ID": qb.player_id if qb is not None else None,
            "V18_PICKED_QB_ID": v18_qb_id,
            "OVERRIDE_CHANGED_PICK": bool(
                qb is not None and v18_qb_id is not None and qb.player_id != v18_qb_id),
        }
    ...
    artifact = {
        "SCHEMA": "SPORTS_NOVA_V19_WEEK1_REPLAY_PREDICTION",
        "GAME_ID": game_id, "SEASON": SEASON, "WEEK": WEEK,
        "AWAY_TEAM": away, "HOME_TEAM": home,
        "KICKOFF_UTC": kickoff.isoformat(),
        "MODEL_VERSION": v19_sim.MODEL_VERSION,
        ...
        "HOME_QB_IDENTITY": override_info(home, home_qb),
        "AWAY_QB_IDENTITY": override_info(away, away_qb),
        "MARKET": market,
...
def main():
    ...
    for r in week1.itertuples():
        ...
        v18_draws_sha = sha256_file(v18_draws_path)

        # Byte-identical draws check: write V19's draws to a temp buffer and hash.
        import io as _io
        buf = _io.BytesIO()
        np.savez_compressed(buf, **draws)
        v19_draws_sha_uncompressed_diff_ok = None  # compression stream itself may differ; compare arrays instead
        v18_draws_npz = np.load(v18_draws_path)
        arrays_match = all(
            k in v18_draws_npz.files and np.array_equal(draws[k], v18_draws_npz[k])
            for k in draws
        ...
        fidelity_log.append({
            ...
            "V19_STATE_HASH": artifact["STATE_HASH"], "V18_STATE_HASH": v18_pred["STATE_HASH"],
        })

    fidelity_all_pass = all(
        f.get("STATE_HASH_MATCH") and f.get("MODEL_SCORE_DIST_MATCH") and f.get("RAW_DRAWS_ARRAYS_MATCH")
        for f in fidelity_log if f["STATUS"] == "CHECKED"
    ) and all(f["STATUS"] == "CHECKED" for f in fidelity_log) and len(fidelity_log) == len(FIDELITY_GAME_IDS)

    print(json.dumps({"FIDELITY_CHECK": fidelity_log, "FIDELITY_ALL_PASS": fidelity_all_pass}, indent=2))
    if not fidelity_all_pass:
        raise SystemExit("BLOCKED: V19 harness does not reproduce frozen V18 byte-for-byte with override disabled.")

    # ---- PHASE 2: real run (override enabled where CONFIRMED) ----
    PRED_DIR.mkdir(parents=True, exist_ok=True)
    DRAWS_DIR.mkdir(parents=True, exist_ok=True)
    capture_log = []
    for r in week1.itertuples():
        gid, result, err = run_one_game(walkforward_mod, v19_sim, panel, r, confirmed_resolutions)
        if err:
    ...
    (REPLAY_DIR / "SPORTS_NOVA_V19_WEEK1_CAPTURE_LOG.json").write_text(json.dumps(capture_log, indent=2))
    print(json.dumps({
