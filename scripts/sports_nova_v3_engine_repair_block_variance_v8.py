"""V8: SN3_SPORTS_V7_BLOCK_VARIANCE repair validation.

Identical to V7 (same 60-game/128-sim cohort, same metrics) except it runs
against the SN3_SPORTS_V7_BLOCK_VARIANCE-repaired engine: simulator.py's
per-block loop used to be hard-capped at exactly 24 blocks/game because (a)
the loop bound was a literal `range(24)` and (b) elapsed time per block was
capped at `min(90, ...)` seconds -- together these made 24*90=2160 seconds
the maximum possible clock advance against the 3600-second game clock, so
`if state.seconds_remaining <= 0: break` could never fire (proof in
SN3_SPORTS_V6_REPORT.json MECHANISM_CHECKS). Fixed by raising the loop bound
to a generous, non-binding safety net (60) and removing the 90s elapsed cap
(replaced with a 500s defensive bound; true max given existing plays<=12 and
pace<=36 clips is 432s) so the ALREADY-INTENDED clock-driven termination
mechanism actually governs block count. No new block-count distribution was
invented: a standalone Monte Carlo check (no worker code touched) confirmed
this alone reproduces the historical combined-blocks/game mean almost
exactly (23.28 sim vs 23.62 historical) with real, non-zero dispersion,
before this fix was even made. TEAM_STAT_NAMES gained a 'blocks' counter
(simulator.py) purely for instrumentation/validation; it does not feed any
other computation. V1-V7 outputs are left untouched for audit.
"""
...
def main():
    ...
    for g in selected:
        ...
        game_blocks = sim.team(sim.team_ids[0], 'blocks') + sim.team(sim.team_ids[1], 'blocks')
        stage.append({'GAME_ID': g, 'SIM_GAME_PLAYS': float(sum(team_plays)), 'SIM_TEAM_PLAYS_MEAN': team_plays,
                      ...
                      'SIM_BLOCKS_MEAN': float(np.mean(game_blocks)), 'SIM_BLOCKS_SD': float(np.std(game_blocks, ddof=1)),
                      'SIM_BLOCKS_MIN': int(np.min(game_blocks)), 'SIM_BLOCKS_MAX': int(np.max(game_blocks)),
                      'SIM_GAME_PLAYS_SD': float(np.std(sim.team(sim.team_ids[0], 'pass_attempts') + sim.team(sim.team_ids[0], 'rush_attempts') +
                                                          sim.team(sim.team_ids[1], 'pass_attempts') + sim.team(sim.team_ids[1], 'rush_attempts'), ddof=1))})
    ...
    all_game_blocks_means = [s['SIM_BLOCKS_MEAN'] for s in stage]
    all_game_plays_sds = [s['SIM_GAME_PLAYS_SD'] for s in stage]
    result['block_count'] = {
        'cross_game_mean_of_per_game_means': float(np.mean(all_game_blocks_means)),
        'cross_game_sd_of_per_game_means': float(np.std(all_game_blocks_means, ddof=1)),
        'mean_within_game_sd_across_sims': float(np.mean([s['SIM_BLOCKS_SD'] for s in stage])),
        'min_observed': int(min(s['SIM_BLOCKS_MIN'] for s in stage)),
        'max_observed': int(max(s['SIM_BLOCKS_MAX'] for s in stage)),
    }
    result['total_plays_sd'] = {
        'mean_within_game_sd_across_sims': float(np.mean(all_game_plays_sds)),
        'cross_game_sd_of_per_game_means': float(np.std([s['SIM_GAME_PLAYS'] for s in stage], ddof=1)),
    }
    (OUT / 'SPORTS_NOVA_V3_ENGINE_REPAIR_VALIDATION_V8.json').write_text(json.dumps(result, indent=2))
