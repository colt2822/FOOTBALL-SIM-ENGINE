"""V4 of the post-repair QB allocation validation.

Identical to V3 (same 60-game/128-sim cohort, same metrics) except it runs
against the SN3_QB_YARDS_CONDITIONAL_V4-repaired engine: simulator._run_one
now samples each receiver's per-catch yards from that receiver's own prior
yards_per_reception feature (already computed causally in make_state, same
as catch_rate) instead of a single global mean/sd shared by every receiver
on every team. V1/V2/V3 outputs are left untouched for audit.
"""
...
def main():
    ...
    (OUT / 'SPORTS_NOVA_V3_ENGINE_REPAIR_VALIDATION_V4.json').write_text(json.dumps(result, indent=2))
