"""V3 of the post-repair QB allocation validation.

Identical to V2 (same 60-game/128-sim cohort, same metrics) except it runs
against the SN3_QB_ID_ALLOC_V3-repaired engine: simulator._qb_shares now
gates QB eligibility by recency (games_since_last_team_game, populated
causally in make_state) before splitting by volume, instead of letting raw
cumulative volume alone decide the primary QB. V1/V2 outputs are left
untouched for audit.
"""
...
def main():
    ...
    (OUT / 'SPORTS_NOVA_V3_ENGINE_REPAIR_VALIDATION_V3.json').write_text(json.dumps(result, indent=2))
