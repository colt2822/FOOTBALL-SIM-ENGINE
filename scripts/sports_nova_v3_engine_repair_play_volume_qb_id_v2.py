"""V2 of the post-repair QB allocation validation.

Same 60-game / 128-sim cohort as sports_nova_v3_engine_repair_play_volume_qb_id_v1.py
(V1 is left untouched for audit). This version:

  1. Fixes the identity metric. V1's `identity_match_rate` compared the
     engine's projection only against QBs already present in the box score,
     so a lone evaluated backup with zero simulated attempts still counted as
     a "match" by construction (max() over a size-1 list). This version
     compares the engine's actual per-team-game starter selection
     (`_primary_qb`, the argmax of the causal QB share model) against the
     realized argmax-attempts QB in the box score.
  2. Adds explicit QB-level conservation checks (gate 3): per simulation,
     per team, sum of QB attempts <= team pass attempts, and no player is
     double counted across teams.
  3. Reports full attempt distributions (P10-P90) and >42/>50-attempt rates
     for realized and simulated attempts, not just means, per gate 8.
  4. Reports QB_UNKNOWN separately from wrong-selection zeros (gate 4/7).
"""
...
def main():
    ...
    (OUT / 'SPORTS_NOVA_V3_ENGINE_REPAIR_VALIDATION_V2.json').write_text(json.dumps(result, indent=2))
